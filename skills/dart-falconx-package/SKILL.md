...
dependencies:
  dart_falconx:                      # or only dart_falconnect / dart_falmodel / dart_faltool
    git:
      ...
      ref: <latest_tag>              # e.g. 2.1.1 — see `git ls-remote --tags`
...
| Interceptors                                    | `RequestStampInterceptor`, `TokenRefreshInterceptor` (sharing an `AuthSession`), `RetryInterceptor`, `CacheInterceptor`, `ConcurrencyLimitInterceptor`, `TokenBucketRateLimitInterceptor`, `RetryAfterPauseInterceptor`, `HttpLogInterceptor`, `HttpJsonLogInterceptor`, `NetworkExceptionHandlerInterceptor`, `HttpClientConfig`; the cache, limiter, retry, and log interceptors take their own config box, the two auth interceptors an `AuthSession`                                                                                                                            | dart_falconnect             | `references/http.md`        |
