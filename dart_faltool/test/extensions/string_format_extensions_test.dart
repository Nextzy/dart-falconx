  ...
  group('FalconToolStringFormatExtension', () {
    ...
    group('String Formatting', () {
      ...
      test('truncate truncates string to specified length', () {
        ...
        expect('Hello World'.truncate(null), 'Hello World');
      });
    ...
    group('Masking', () {
      test('maskPhoneNumber masks middle digits', () {
        ...
        expect('+66891234567'.maskPhoneNumber(), '+66*******67');
        expect('089-123-4567'.maskPhoneNumber(), '08******67');
      });

      test('maskPhoneNumber with custom lengths', () {
        expect(
          '0891234567'.maskPhoneNumber(prefixLength: 3, suffixLength: 4),
          '089***4567',
        );
        expect(
          '0891234567'.maskPhoneNumber(prefixLength: 4, suffixLength: 4),
          '0891**4567',
        );
      });

      test('maskPhoneNumber with custom mask char', () {
        expect('0891234567'.maskPhoneNumber(maskChar: '#'), '08######67');
      });

      test('maskPhoneNumber returns original if too short', () {
        expect('1234'.maskPhoneNumber(), '1234');
        expect(
          '12345'.maskPhoneNumber(prefixLength: 3, suffixLength: 3),
          '12345',
        );
      });

      test('maskEmail masks local part', () {
        expect('user@example.com'.maskEmail(), 'us**@example.com');
        expect('john.doe@gmail.com'.maskEmail(), 'jo******@gmail.com');
      });

      test('maskEmail with short local part', () {
        expect('a@example.com'.maskEmail(), '*@example.com');
        expect('ab@example.com'.maskEmail(), '**@example.com');
      });
      ...
      test('maskEmail with custom mask char', () {
        expect('user@example.com'.maskEmail(maskChar: '#'), 'us##@example.com');
      });
