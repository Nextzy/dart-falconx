...
4. When `options.cancelToken` is already cancelled, reject with the token's own cancel error, re-targeted to this request's options; the type and the reason stay as dio set them.
